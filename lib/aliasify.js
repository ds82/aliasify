...
(function() {
  ...
  getReplacement = function(file, aliases, regexps, opts) {
  ...
  makeTransform = function(requireAliases) {
    return transformTools.makeFunctionTransform(TRANSFORM_NAME, {
      ...
      var absolutePaths, aliases, arg, configDir, err, error, error1, file, fileDir, i, len, ref, regexps, remainingArgs, replacement, result, verbose;
      if (!opts.config) {
  ...
  module.exports.getReplacement = function(alias, config) {
    var aliases, configDir, regexps, replacement;
    if (!config) {
      return new Error("Could not find configuration for aliasify");
    }
    ...
    if (replacement != null) {
      ...
      return path.resolve(configDir, replacement);
    } else {
      return false;
    }
  };
